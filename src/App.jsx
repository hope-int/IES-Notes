...
function App() {
  ...
  const handleRegistrationComplete = React.useCallback(async (profile) => {
  ...
  const fetchDepartments = async (retryCount = 0) => {
    ...
    try {
      ...
      if (error) {
        if (retryCount < 2) return setTimeout(() => fetchDepartments(retryCount + 1), 2000);
        throw error;
      }
      setDepartments(data || []);
    } catch (error) {
  ...
  const fetchSemesters = async (deptId, retryCount = 0) => {
    ...
    try {
      const { data, error } = await Promise.race([promise, timeout]);
      if (error) {
        if (retryCount < 2) return setTimeout(() => fetchSemesters(deptId, retryCount + 1), 2000);
        throw error;
      }
      setSemesters(data || []);
    } catch (error) {
      console.warn('Semesters fetch failed:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchSubjects = async (semId, retryCount = 0) => {
    ...
    try {
      const { data, error } = await Promise.race([promise, timeout]);
      if (error) {
        if (retryCount < 2) return setTimeout(() => fetchSubjects(semId, retryCount + 1), 2000);
        throw error;
      }
      setSubjects(data || []);
    } catch (error) {
      console.warn('Subjects fetch failed:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchNotes = async (subjectId, retryCount = 0) => {
    ...
    try {
      const { data, error } = await Promise.race([promise, timeout]);
      if (error) {
        if (retryCount < 2) return setTimeout(() => fetchNotes(subjectId, retryCount + 1), 2000);
        throw error;
      }
      setNotes(data || []);
    } catch (error) {
  ...
  // Show Registration if not logged in (and splash is done)
  const showRegistration = !initializing && !session && !userProfile;
  ...
  return (
    <>
      ...
      {/* Main App - always rendered underneath, avoids white flash */}
      {!showRegistration && (
        <>
          <InstallPWA />
          <AnimatePresence>
                        ...
                        <div className="row mb-5 align-items-center justify-content-center g-3">
                          <div className="col-12 d-flex align-items-center justify-content-center gap-3 flex-wrap">
                            ...
                            {navStack.length > 1 && (
                              <button
                                onClick={handleBack}
                                className="clay-button rounded-circle p-0 d-flex align-items-center justify-content-center"
                                style={{ width: '48px', height: '48px', minWidth: '48px', background: 'var(--bg-card)' }}
                              >
                            ...
                            {!loading && filteredItems.length === 0 && (
                              <div className="col-12 py-5 text-center">
                                <motion.div
                                  ...
                                  className="clay-card p-5 d-inline-block mx-auto shadow-sm"
                                  style={{ maxWidth: '400px', background: 'var(--bg-card)', border: '1px solid rgba(0,0,0,0.05)' }}
                                >
                                  <div className="bg-primary bg-opacity-10 p-4 rounded-circle d-inline-block mb-4">
                                    <Search size={48} className="text-primary" />
                                  </div>
                                  <h3 className="fw-bold text-dark mb-2">No Items Found</h3>
                                  <p className="text-muted mb-4 px-3 small">We couldn't find any resources for this section. Try exploring all departments instead?</p>
