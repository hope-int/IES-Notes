...
const SkeletonLoader = ({ type = 'text', count = 1, className = '', style = {} }) => {
    ...
    const getStyles = () => {
        ...
        if (type === 'image') return { ...baseStyle, height: '200px', width: '100%', marginBottom: '1rem' };
        if (type === 'avatar') return { ...baseStyle, height: '40px', width: '40px', borderRadius: '50%' };
        if (type === 'card') return { ...baseStyle, height: '150px', width: '100%', borderRadius: '12px' };
